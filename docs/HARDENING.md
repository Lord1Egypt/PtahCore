...
| `check_power_grid` at chip level never converges: >12 GB RSS+swap, silent, killed | chip_top | the PG connectivity graph of the full chip (giant macro pin straps + 16 SRAMs + strip rails) exceeds this 7.7 GB VM — same class as the grid-level GRT OOM, but swap doesn't save it | **RECORDED, NOT VERIFIED BY PSM** — power connectivity evidence at chip level: pdngen completes clean (its own unconnected-shape pruning reports nothing), only two 0.1% partial-block warnings, and route-stage DRC checks the PG via stacks physically. Re-run `check_power_grid` on ≥16 GB hardware before any tapeout-grade claim |
| `DPL-0036` detailed placement fails — 10 repair buffers unplaceable, all at x≈3.1 µm (the west sliver) | chip_top | post-resize hold/setup buffers land between the die edge and the SRAM column where there's no legal site within the default search radius | `DETAIL_PLACEMENT_ARGS = -max_displacement 50` — lets them hop to the escape rows just east of the SRAM column. `500` made the diamond search degenerate (hours, no progress); 50 is enough |
| `DPL-0036` recurs (1 buffer stranded INSIDE the grid macro) after the launch-tap-buffer RTL; displacement is a dead end — 150 µm finishes-but-fails-by-1, 170 µm AND 250 µm both DEGENERATE (diamond search runs hours at 100% CPU, no progress, killed) | chip_top | ROOT CAUSE is the floorplan, not the search radius: the stdcell strips held ~96k µm² of cells in ~285k µm² (~33% util) — too tight for repair buffers to legalize, so they strand and the legalizer searches forever for room. The displacement window that both *finishes* and *places all* doesn't exist for this netlist | widen the strips in `gen_floorplan.py`: W_STRIP 172.8→220.32, N_STRIP 43.2→64.8 (~25% util) → cells legalize at small displacement, no degeneration. **The lever for stranded-repair-buffer DPL failures is FLOORPLAN AREA, not max_displacement.** Sharp cliff: >150 µm degenerates here |
| CTS (4_1) dies: `ODB-0370` disconnect dont_touch spine buffer / `RSZ-3006` buffer clk_spine root / `RSZ-0060` max buffers (71,935 inserted, "51,790 hold viol", WNS −152,431 ps) | chip_top | **the −152 ns is a PHANTOM** — offline STA on 3_place.odb (propagated clocks, /tmp/sta_hold.tcl) shows real slack is setup −1255 / hold −1329 ps. The catastrophe is one net: **`clk_s_tap`, the stage-A launch clock, fans out to 806 registers UNBUFFERED** (confirmed via getNets fanout scan, /tmp/sta_spine.tcl). estimate_parasitics on an 806-sink chip-wide net → astronomical capture-clock RC → −152 ns. CTS can't tree it: `-clk_nets clk` excludes the taps; unscoped CTS / its repair tries to edit the dont_touch spine backbone | **clk_s PART FIXED, deeper issue exposed:** (1) `u_clk_s_drv` (ptah_clkbuf) in chip_top.sv isolates a fresh `clk_s_tap` net from the spine so CTS can build a real distribution tree below it (a bare tap net post-flatten IS the dont_touch spine net — ODB-0373). (2) SDC dont_touch narrowed `ref_name == BUFx24` → `name =~ *u_spine_*`. (3) `CTS_ARGS = -sink_clustering_enable -clk_nets {clk clk_s_tap}`, dropping −repair_clock_nets (RSZ-3006). Bit-exact (ptah_clkbuf = wire in sim). **RESULT: CTS now TREES clk_s_tap cleanly (784 sinks → 13-buffer H-tree, no ODB-037x) — the clk_s phantom is gone.** BUT post-CTS hold repair still hit RSZ-0060: worst hold moved to `launch_b[31].b_q[31]` (−98 ns), the STAGE-B launch banks on the un-buffered `wtap`/`ntap` taps. My "~41 localized fanout, fine" assumption was WRONG — same RC-phantom disease, just not the worst before |
| **OPEN — the stage-B launch-clock fork (the real 7d-3 chip-clock problem):** the per-row/col `wtap`/`ntap` spine taps drive the stage-B launch banks un-buffered → −98 ns RC phantom (51,825 hold endpoints). Can't just tree them like clk_s: (a) `wtap` is SHARED — `clk_row_v[0..31]=wtap[0..31]` (grid macro row clocks, MUST keep the +82 ps/row traveling stagger) and `clk_lw_v=wtap[17..48]` (launch) — treeing wtap destroys the grid delivery; (b) stage-B launches data INTO the grid's `b_n_flat`, captured by the grid's internal staggered column clock — generic CTS zero-skew would violate the grid lib's setup/hold arcs (unfixable, the grid is a macro). **The honest fix: keep the taps un-treed (stagger preserved) but make their fanout LOCAL via placement — cluster each launch_b[j]/launch_w[i] group at its grid column/row edge so a 32-fanout tap net is short, low-RC, no phantom.** Needs floorplan placement regions/groups (+ likely RTL separation of grid-delivery taps from launch taps so each can be handled independently). Deferred — design decision, see STEPS.md |
